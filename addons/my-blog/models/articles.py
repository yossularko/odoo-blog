...
class BlogArticles(models.Model):
    ...
    # Method untuk quick tag creation
    def create_tag_from_string(self, tag_string):
        """Create tags from comma-separated string"""
        if not tag_string:
            return self.env['blog.tag']
        
        tag_names = [name.strip() for name in tag_string.split(',')]
        tags = self.env['blog.tag']
        
        for tag_name in tag_names:
            if tag_name:
                # Format tag name
                formatted_name = tag_name.lower().replace(' ', '-')
                
                # Find or create tag
                tag = self.env['blog.tag'].search([('name', '=', formatted_name)], limit=1)
                if not tag:
                    tag = self.env['blog.tag'].create({'name': formatted_name})
                tags |= tag
        
        return tags
    
    # Enhanced copy method untuk handle categories & tags
    def copy(self, default=None):
        """Override copy to handle categories & tags"""
        if default is None:
            default = {}
